...
public class BookDAOImpl implements BookDAO {
	...
	@Override
	public HashMap<String, List<String>> getBooksAndTags() {
		...
		try {
			connection = DAOUtilities.getConnection();
			String sql = "Select title, book_tags.tag_name from public.books full outer join public.book_tags on book_tags.isbn_13 = books.isbn_13";
			stmt = connection.prepareStatement(sql);
			ResultSet rs = stmt.executeQuery();			// Queries the database
			...
			while (rs.next()) {
				...
				Tag tag = new Tag();
							...
							// Each variable in our Book object maps to a column in a row from our results.
							// Need to connect tags of the same book
				
				// book.setAuthor(rs.getString("author"));
				// get the book
				// get a tag for that book
				// get the next tag(s) if applicable
				// push into array
				// move to next row
					book.setTitle(rs.getString("title")); 
					...
					books.put(rs.getString("title"), list);
					
					books.get(rs.getString("title")).add(rs.getString("tag_name"));
